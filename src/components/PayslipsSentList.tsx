...
export default function PayslipsSentList({
  ...
  if (loading) {
    return (
      <div className="text-slate-700 py-6 text-center text-lg font-medium uppercase tracking-wide">
    ...
    <div className="space-y-4">
      {showFilters && (
        <div className="flex flex-wrap gap-3 items-end rounded-xl border border-cyan-200/80 bg-white p-4 shadow-sm">
          <div>
            <label className="block text-lg font-medium uppercase tracking-wide text-slate-800 mb-1">Employee</label>
            <select
              value={filterEmployee}
              onChange={(e) => setFilterEmployee(e.target.value)}
              className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-900 text-lg focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400"
            >
          ...
          <div>
            <label className="block text-lg font-medium uppercase tracking-wide text-slate-800 mb-1">From (date)</label>
            <input
              type="date"
              value={filterFrom}
          ...
          <div>
            <label className="block text-lg font-medium uppercase tracking-wide text-slate-800 mb-1">To (date)</label>
            <input
              type="date"
              value={filterTo}
      ...
      {payslips.length === 0 ? (
        <div className="rounded-xl border border-cyan-200/80 bg-white p-6 text-center text-slate-700 text-lg">
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              <p className="font-medium uppercase tracking-wide text-slate-900 text-lg">{p.employeeName}</p>
              <p className="text-lg text-slate-700 truncate">{p.employeeEmail}</p>
              <p className="text-lg text-slate-700 mt-1">{p.fileName}</p>
              {p.emailMessage && (
                <p className="text-lg text-slate-600 mt-2 line-clamp-2 border-l-2 border-slate-200 pl-2">
                  {p.emailMessage}
                </p>
              )}
            </div>
            <time
              ...
              className="text-lg text-slate-600 shrink-0 whitespace-nowrap font-medium uppercase tracking-wide"
            >
